...
public class MainActivity extends ActionBarActivity implements ScrollTabHolder {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			mTopImage.setTranslationY(savedInstanceState
					.getFloat(IMAGE_TRANSLATION_Y));
			mHeader.setTranslationY(savedInstanceState
...
//		mViewPager.setOffscreenPageLimit(mNumFragments);
		...
		ViewPager.OnPageChangeListener listener = new ViewPager.OnPageChangeListener() {
			...
			public void onPageScrolled(int position, float positionOffset,
				...
				if (positionOffsetPixels > 0) {
					...
					if (position < currentItem) {
					 ...
					 Log.e(TAG, "header height " + mHeader.getHeight() +
			...
			public void onPageSelected(int position) {
				...
				if (scrollTabHolders == null
						|| scrollTabHolders.size() != mNumFragments) {
					return;
				}
				...
				ScrollTabHolder currentHolder = scrollTabHolders
						.valueAt(position);
				currentHolder
						.adjustScroll((int) (mHeader.getHeight() + mHeader
								.getTranslationY()), mHeader.getHeight());
			}
	...
	private int getScrollY(AbsListView view) {
		...
		int firstVisiblePosition = view.getFirstVisiblePosition(); // �ɼ�
